...
use crate::{EntityPtr, error::Error, process::ProcessMemory, remote_ptr::RemotePtr};
...
fn read_res_ref(
    ...
    match null_byte {
        Some(_) => {
            let cs = CStr::from_bytes_until_nul(&bytes).map_err(|e| Error::InvalidString {
                msg: e.to_string(),
                bytes: bytes.clone(),
            })?;
